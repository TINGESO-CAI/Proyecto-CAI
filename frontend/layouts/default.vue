	...
	methods:{
		...
		*/
		async ingresar(){
			...
			try {
				let response = await axios.post('http://'+process.env.IP_FRONT+':5000/entrar',credenciales);
				//limpiar
				this.correo = '';
				this.contrasena = '';
		...
		async traerDatos(){
			this.message = '';
			try {
				let response = await axios.get('http://'+process.env.IP_FRONT+':5000/cuenta/permisos');
				//limpiar
		...
		async salir(){
			...
			this.message = '';
			try {
				let response = await axios.post('http://'+process.env.IP_FRONT+':5000/salir');
				//limpiar
				this.usuario = {};
				this.contrasena = '';
				this.successMessage();
